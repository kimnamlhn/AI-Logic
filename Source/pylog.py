...
# match once you tell it to. Pylog currently returns all possible matches.
#
############################################################
...
from pyparsing import (Group, Keyword, NoMatch, Suppress, Word, ZeroOrMore,
                       alphas)
            ...
            if result["result"] == "query":
                ...
                if cont_vars:
                    ...
                    if len(constants) != len(relations[name][0]):
                        print("Error: Fact {} takes {} argument(s), {} given.".format(
                            name, len(relations[name][0]), len(constants)))
                    else:
